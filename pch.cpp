...
public:
  SimpleDXGI(HWND);
  ~SimpleDXGI();
  std::unique_ptr<cv::Mat> CaptureWindow(int, int, int, int);
  std::unique_ptr<cv::Mat> grabByRegion(UINT left, UINT top, UINT width,
                                        UINT height);
...
SimpleDXGI::~SimpleDXGI() { CloseSession(); }
...
void SimpleDXGI::InitSession() {
  ...
  winrt::check_hresult(idxgiDevice2->GetParent(winrt::guid_of<IDXGIAdapter>(),
                                               adapter.put_void()));
  winrt::check_hresult(
  ...
  RECT rect{};
  DwmGetWindowAttribute(hwndTarget, DWMWA_EXTENDED_FRAME_BOUNDS, &rect,
                        sizeof(RECT));
  const auto size = winrt::Windows::Graphics::SizeInt32{rect.right - rect.left,
...
std::unique_ptr<cv::Mat> SimpleDXGI::CaptureWindow(int left, int top, int width,
                                                   int height) {
  while (!isFrameArrived || nullptr == texture) {
    std::this_thread::sleep_for(std::chrono::milliseconds(1));
  }
  isFrameArrived = false;
  auto pData = grabByRegion(left, top, width, height);
  return pData;
}

bool safe_get_texture_desc(winrt::com_ptr<ID3D11Texture2D> *texture,
...
std::unique_ptr<cv::Mat> SimpleDXGI::grabByRegion(UINT left, UINT top,
  ...
  winrt::check_hresult(d3dDevice->CreateTexture2D(&capturedTextureDesc, NULL,
                                                  userTexture.put()));
  ...
  sourceRegion.back = 1;
  d3dContext->CopySubresourceRegion(userTexture.get(), 0, 0, 0, 0,
                                    texture.get(), 0, &sourceRegion);

  D3D11_MAPPED_SUBRESOURCE resource;
  winrt::check_hresult(
      d3dContext->Map(userTexture.get(), NULL, D3D11_MAP_READ, 0, &resource));
  d3dContext->Unmap(userTexture.get(), NULL);

  UINT lBmpRowPitch = width * 4;
  auto sptr = static_cast<BYTE *>(resource.pData);

  auto mat = std::make_unique<cv::Mat>(height, width, CV_8UC4);

  UINT lRowPitch = std::min<UINT>(lBmpRowPitch, resource.RowPitch);

  for (size_t h = 0; h < height; ++h) {
    avx2_memcpy(mat->ptr(h), sptr, lRowPitch);
...
void init_dxgi(HWND hwnd) { dxgi = new SimpleDXGI(hwnd); }

std::unique_ptr<cv::Mat> grab(int left, int top, int width, int height) {
  if (dxgi) {
    return dxgi->CaptureWindow(left, top, width, height);
  }
  return nullptr;
}
